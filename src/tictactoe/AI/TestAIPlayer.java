...
public class TestAIPlayer {
      ...
      public int decideMove(){
        ...
        int index=0,maxScore=Integer.MIN_VALUE,winIndex=0,curScore;
        Score currentScore;
        ...
        for(PlayerAI currentPlayer:_allPlayers){
            ...
            for(int i=0;i<9;i++){                                
                    if(currentScore.score[i][0]>=1){
                        System.out.println("Win status ["+10*currentScore.score[i][0]*(i+1)+"] "+(index+1)); 
                        winner[index]=10*currentScore.score[i][0]*(i+1);break;
                    }else if(currentScore.score[i][1]>=1){
                        System.out.println("Draw status ["+(i+1)*currentScore.score[i][1]+"] "+(index+1)); 
                        winner[index]=currentScore.score[i][1]*(i+1);break;
                    }
                    else if(currentScore.score[i][2]>=1){
                        System.out.println("Loose status ["+(-10)*currentScore.score[i][2]*(i+1)+"] "+(index+1)); 
                        winner[index]=-10*currentScore.score[i][2]*(i+1);break;
                    }
            }
            index++;
        }
        ...
        {
            System.out.println("Current score of [ "+i+" ] is : "+winner[i]+", MAX SCORE:"+maxScore);
            if(maxScore<winner[i]){
                maxScore=winner[i];
                winIndex=i;
            }
        }
