...
//    PLAYERTYPE currentAiTurn;
    ...
    private List<Integer> getUnCoveredMoves(COINNAME[][] _currentGameState){
        ...
        for(Score currentScore:allScores){            
            for(int i=0;i<9;i++){
                ...
                int looseScore=currentScore.score[i][2];
                    if(WinScore>=1){
                        System.out.println("Win status ["+10*WinScore*(i+1)+"] "+(index+1)); 
                        winner[index]=-10*WinScore*(i+1);break;
                    }else if(drawScore>=1){
                        System.out.println("Draw status ["+drawScore*(i+1)+"] "+(index+1)); 
                        winner[index]=drawScore*(i+1);break;
                    }
                    else if(looseScore>=1){
                        System.out.println("Loose status ["+(-10)*looseScore*(i+1)+"] "+(index+1)); 
                        winner[index]=10*looseScore*(i+1);break;
                    }
            }
            index++;
        }        
